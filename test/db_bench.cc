...
class Stats {
 ...
 public:
  ...
  void Stop() {
    finish_ = Timestamp::now();
    seconds_ = time_interval(finish_, start_);
  }
  ...
  void FinishedSingleOp() {
    if (FLAGS_histogram) {
      ...
      double micros = time_interval(now, last_op_finish_) * Timestamp::kMicroPerSecond;
      hist_.Add(micros);
      if (micros > 20000) {
  ...
  void Report(const Slice& name) {
    ...
    double elapsed = time_interval(finish_, start_);
...
private:
  ...
  void PrintEnvironment() {
    fprintf(stderr, "yodb:    Alpha version\n");
...
private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      Thread *thr = new Thread(boost::bind(&Benchmark::ThreadBody, &arg[i]));
      thr->run();
    }
...
#endif
    if (FLAGS_cache_size) {
        opts.cache_limited_memory = FLAGS_cache_size;
    }

    char file_name[100];
    db_num_++;
    snprintf(file_name, sizeof(file_name),
             "dbbench_yodb_%d", db_num_);
